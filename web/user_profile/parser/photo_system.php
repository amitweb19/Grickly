...
?><?php 
if (isset($_FILES["profilepic"]["name"]) && $_FILES["profilepic"]["tmp_name"] != ""){
	...
	if($width < 10 || $height < 10){
		header("location: ../../../functions/message.php?msg=ERROR: That image has no dimensions");
        exit();	
	}
	$db_file_name = rand(100000000000,999999999999).".".$fileExt;
	if($fileSize > 1048576) {
		header("location: ../../../functions/message.php?msg=ERROR: Your image file was larger than 1mb");
		exit();	
	} else if (!preg_match("/\.(gif|jpg|png)$/i", $fileName) ) {
		header("location: ../../../functions/message.php?msg=ERROR: Your image file was not jpg, gif or png type");
		exit();
	} else if ($fileErrorMsg == 1) {
		header("location: ../../../functions/message.php?msg=ERROR: An unknown error occurred");
		exit();
	}
	$sql = "SELECT profilepic FROM users WHERE username='$log_username' LIMIT 1";
	$query = mysqli_query($db_conx, $sql);
	$row = mysqli_fetch_row($query);
	$profilepic = $row[0];
	if($profilepic != ""){
		$picurl = "../../../users/$log_username/$profilepic"; 
	    if (file_exists($picurl)) { unlink($picurl); }
	}
	$moveResult = move_uploaded_file($fileTmpLoc, "../../../users/$log_username/$db_file_name");
	if ($moveResult != true) {
		header("location: ../../../functions/message.php?msg=ERROR: File upload failed");
		exit();
	}
	...
	$sql = "UPDATE users SET profilepic='$db_file_name' WHERE username='$log_username' LIMIT 1";
	$query = mysqli_query($db_conx, $sql);
	mysqli_close($db_conx);
	header("location: ../../user_profile/profile.php?u=$log_username");
	exit();
}
?><?php 
if (isset($_FILES["photo"]["name"]) && isset($_POST["gallery"])){
	...
	$row = mysqli_fetch_row($query);
	if($row[0] > 14){
		header("location: ../../../functions/message.php?msg=The demo system allows only 15 pictures total");
        exit();	
	}
	$gallery = preg_replace('#[^a-z 0-9,]#i', '', $_POST["gallery"]);
